...
describe('Velixar', () => {
  ...
  describe('store', () => {
    it('should store memory successfully', async () => {
      ...
      expect(mockFetch).toHaveBeenCalledWith(
        'https://api.velixarai.com/memory',
  ...
  describe('search', () => {
    it('should search memories successfully', async () => {
      ...
      expect(mockFetch).toHaveBeenCalledWith(
        'https://api.velixarai.com/memory/search?q=test+query',
        expect.objectContaining({
          headers: expect.objectContaining({
            'Authorization': 'Bearer test-key'
          })
    ...
    it('should handle search with options', async () => {
      ...
      expect(mockFetch).toHaveBeenCalledWith(
        'https://api.velixarai.com/memory/search?q=test+query&user_id=user-1&limit=5',
        expect.any(Object)
      );
    });
  });
  ...
  describe('get', () => {
    it('should get memory by ID', async () => {
      ...
      expect(mockFetch).toHaveBeenCalledWith(
        'https://api.velixarai.com/memory/mem-1',
        expect.objectContaining({
          headers: expect.objectContaining({
            'Authorization': 'Bearer test-key'
          })
